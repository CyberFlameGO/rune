...
pub struct Variant {
    ...
    /// Handler to use to test if a value matches the given variant.
    pub(crate) matches: Option<Arc<FunctionHandler>>,
}
...
impl Module {
        ...
        Func: Function<Args, Return = T>,
    {
        ...
        let variant = match en.variants.get_mut(index) {
            Some((_, variant)) if matches!(variant.kind, VariantKind::Tuple(..)) => variant,
            _ => {
    ...
    /// Register a helper that tests if the given variant matches.
    pub fn variant_matches<Func, Args, T>(
        &mut self,
        index: usize,
        matches: Func,
    ) -> Result<(), ContextError>
    where
        T: Named + TypeOf,
        Func: Function<Args, Return = bool>,
    {
        let type_hash = T::type_hash();
        ...
        variant.matches = Some(Arc::new(move |stack, args| matches.fn_call(stack, args)));
